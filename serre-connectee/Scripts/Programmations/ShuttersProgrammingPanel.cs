...
public partial class ShuttersProgrammingPanel : Panel
{
	...
	public void Loading(Godot.Collections.Array<string> Data)
	{
		...
		GetNode<OptionButton>("ContainerBlocs/ActionSinon/OptionButtonSinon").Selected = int.Parse(Data[4]);
	...
	/// <summary>
	/// Determines if the panel is programmed and all conditions are correctly set.
	/// </summary>
	...
	public double[] GetValuesProg()
	{
		if (GetIsProgrammed())
		{
			...
			{
				CurrentArray[0] = OptionButtonActionIf.GetSelectedId();
				CurrentArray[2] = OptionButtonActionIfNot.GetSelectedId();
			}
			else if (OptionButtonIf.GetSelectedId() == 1)
			{
				CurrentArray[2] = OptionButtonActionIf.GetSelectedId();
				CurrentArray[0] = OptionButtonActionIfNot.GetSelectedId();
			}
			return CurrentArray;
		}
		return null;
